<?php
    ...
    function DetailData($data){
     ...
     function TambahData($data){
        try {
            $sql = "INSERT INTO pute_group SET  Nama = :Nama, Deskripsi = :Deskripsi";
            $query = $GLOBALS['db']->prepare($sql);
            $query->bindParam("Nama", $data['Nama']);
            $query->bindParam("Deskripsi", $data['Deskripsi']);
            $query->execute();
            $msg['status'] = "sukses";
            $msg['pesan'] = "Berhasil menambah data Group!";
            return $msg;
     ...
     function UbahData($data){
          try {
            $sql = "UPDATE pute_group SET  Nama = :Nama, Deskripsi = :Deskripsi WHERE Id = :Id";
            $query = $GLOBALS['db']->prepare($sql);
            $query->bindParam("Nama", $data['Nama']);
            $query->bindParam("Deskripsi", $data['Deskripsi']);
            ...
            $query->execute();
            $msg['status'] = "sukses";
            $msg['pesan'] = "Berhasil mengubah data Group!";
            return $msg;
        } catch (PDOException $e) {
            $msg['status'] = "gagal";
            $msg['pesan'] = $e->getMessage();
    ...
    function HapusData($data){
        $koneksi = $GLOBALS['db'];
        if(is_array($data)){
            $sql = "DELETE FROM pute_group WHERE Id = :Id";
            $exc = $koneksi->prepare($sql);
            $exc->bindParam('Id', $data['Id'], PDO::PARAM_INT);
            $exc->execute();
            if($exc){
                $result['status'] = "sukses";
                $result['pesan'] = "Berhasil menghapus data Group!";
            }else{
                $result['status'] = "gagal";
    ...
    function ShowData($Id){
        $koneksi = $GLOBALS['db'];
        $sql = "SELECT * FROM pute_group WHERE Id = :Id";
        $exc = $koneksi->prepare($sql);
