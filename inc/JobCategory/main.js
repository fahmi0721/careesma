...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/JobCategory/proses.php?proses=DetailData",
...
function Clear() {
	...
	$("#BtnControl").show();
	$(".select-group").val(null).trigger("change");
	...
	if (Id) {
		if (Status == "ubah") {
			$.ajax({
				type: "POST",
				dataType: "json",
				url: "inc/JobCategory/proses.php?proses=ShowData",
				data: "Id=" + Id,
				...
				success: function (data) {
					console.log(data);
					$("#Title").html("Ubah Kategori Pekerjaan");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#BtnControl").hide();
					$("#aksi").val("update");
					var iForm = ['Id','Nama', 'Deskripsi'];
			...
			$("#aksi").val('delete');
			$("#Id").val(Id)
			$("#proses_del").html("<div class='alert alert-danger'>Apakah anda yakin menghapus data ini ?</div>");
		}
	} else {
		$("#Title").html("Tambah Kategori Pekerjaan");
		$("#FormData").show();
		$("#DetailData").hide();
		$("#proses").html("");
		$("#Nama").focus();
...
function Validasi() {
	var aksi = $("#aksi").val();
	var iForm = ["Nama"];
	var KetiForm = ["Kategori Pekerjaan belum lengkap"];
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "" || $("#" + iForm[i]).val() == null) {
				Customerror("Kategori Pekerjaan", "002", KetiForm[i], 'proses'); $("#" + iForm[i]).focus(); scrolltop(); return false; 
			}
		}
	}
...
function SubmitData() {
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/JobCategory/proses.php?proses=Crud",
			data: data,
			beforeSend: function () {
				StartLoad();
			},
			success: function (result) {
				...
				if (res['status'] == 'sukses') {
					Clear();
					Customsukses("Kategori Pekerjaan", '002', res['pesan'], 'proses');
					LoadData();
					StopLoad();
				} else {
					Customerror("Kategori Pekerjaan", "002", res['pesan'], 'proses');
					StopLoad();
				}
			},
			error: function (er) {
