...
function LoadData(page) {
	...
	$.ajax({
		type: "POST",
		url: "inc/Text/proses.php?proses=DetailData",
...
function Clear() {
	...
	$("#BtnControl").show();
	$('div[for="IsiText"]').find('iframe').contents().find('.wysihtml5-editor').html(null);
	ClearModal();
	...
	if (Id) {
		if (Status == "ubah") {
			$.ajax({
				type: "POST",
				dataType: "json",
				url: "inc/Text/proses.php?proses=ShowData",
				data: "Id=" + Id,
				...
				success: function (data) {
					console.log(data);
					$("#Title").html("Ubah Data Text");
					$("#FormData").show();
					$("#DetailData").hide();
					$("#BtnControl").hide();
					$("#aksi").val("update");
					var iForm = ['Id', 'Judul', 'IsiText'];
...
function Validasi() {
	var aksi = $("#aksi").val();
	var iForm = ["Judul","IsiText"];
	var KetiForm = ["Judul belum lengkap!","Isi Text belum lengkap!"];
	for (var i = 0; i < iForm.length; i++) {
		if (aksi != "delete") {
			if ($("#" + iForm[i]).val() == "" || $("#" + iForm[i]).val() == null) {
				Customerror("Pesan", "002", KetiForm[i], 'proses'); $("#" + iForm[i]).focus(); scrolltop(); return false; 
			}
		}
	}
...
function SubmitData() {
	if (Validasi() != false) {
		...
		$.ajax({
			type: "POST",
			url: "inc/Text/proses.php?proses=Crud",
			data: data,
			beforeSend: function () {
				StartLoad();
			},
