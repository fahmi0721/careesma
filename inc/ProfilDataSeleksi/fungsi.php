<?php
    function LoadDataDiri($NoKtp){
        $koneksi = $GLOBALS['db'];
        $query = $koneksi->prepare("SELECT * FROM careesma_data_diri WHERE NoKtp = :NoKtp");
    ...
    function UpdateLulus($data){
        $sql = "INSERT INTO  careesma_lulus SET IdJobVacancy = '$data[IdLowongan]', IdUser = '$data[IdUser]', Keterangan = '$data[Ket]'";
        $query = $GLOBALS['db']->query($sql);
        if($query){
            $res['status'] = "OK";
    ...
    function HapusLulus($data){
        $sql = "DELETE FROM careesma_lulus WHERE IdJobVacancy = '$data[IdLowongan]' AND IdUser = '$data[IdUser]'";
        $query = $GLOBALS['db']->query($sql);
        if($query){
            $res['status'] = "OK";
            $res['message'] = "Berhasil membatalkan data";
            return $res;
        }else{
            $res['status'] = "GAGAL";
            $res['message'] = "Gagal";
    ...
    function SertifikasiJum(){
        $IdUser =$_SESSION['IdUser'];
        $sql = "SELECT COUNT(Id) as tot FROM careesma_sertifikasi WHERE IdUser = '$IdUser' GROUP BY IdUser";
        $query = $GLOBALS['db']->query($sql);
        $r = $query->fetch(PDO::FETCH_ASSOC);
    ...
    function PkJum(){
        $IdUser =$_SESSION['IdUser'];
        $sql = "SELECT COUNT(Id) as tot FROM  careesma_pengalaman_kerja WHERE IdUser = '$IdUser' GROUP BY Id";
        $query = $GLOBALS['db']->query($sql);
        $r = $query->fetch(PDO::FETCH_ASSOC);
        $res = empty($r['tot']) ? 0 : $r['tot'];
        return $res;
